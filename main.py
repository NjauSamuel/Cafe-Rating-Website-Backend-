...
def get_random_cafe():
    ...
    return jsonify(cafe={key: getattr(random_cafe, key) for key in random_cafe.__table__.columns.keys()})
...
def get_cafe_at_location():
    ...
    all_cafes = result.scalars().all()
    if all_cafes:
        return jsonify(cafes=[{column.name: getattr(cafe, column.name) for column in Cafe.__table__.columns} for cafe in
                              all_cafes])
    else:
